...
`;
    ...
    try {
      const result = await claudeService.moderateContent(this.systemPrompt, userMessage, {
        contentType: 'offer',
        contentId: offer.id,
      });
...
`;
  ...
  async analyzeBatch(items, type = 'offer') {
    ...
    const results = await Promise.all(items.map((item) => analyzeFunc.call(this, item)));
