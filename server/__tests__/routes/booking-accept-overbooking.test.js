...
describe('POST /api/bookings/:id/accept - Overbooking Prevention', () => {
  ...
  test('Should prevent overbooking when accepting multiple bookings', async () => {
    ...
    const booking1Check = await query('SELECT status FROM bookings WHERE id = $1', [booking1Id]);
    ...
    const booking2Check = await query('SELECT status FROM bookings WHERE id = $1', [booking2Id]);
