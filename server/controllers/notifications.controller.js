...
exports.registerFcmToken = asyncHandler(async (req, res) => {
  ...
  if (!token) {
    throw new AppError('رمز FCM مطلوب', 400);
  }

  await FcmTokenModel.upsert(userId, token, deviceInfo || null);

  res.status(200).json({
    success: true,
    message: 'تم تسجيل رمز FCM',
  });
});
...
exports.unregisterFcmToken = asyncHandler(async (req, res) => {
  ...
  await FcmTokenModel.remove(userId, token);

  res.status(200).json({
    success: true,
    message: 'تم إلغاء تسجيل رمز FCM',
  });
});
