...
const getUserStats = asyncHandler(async (req, res) => {
  ...
  // Get total offers
  const offersResult = await query('SELECT COUNT(*) as total FROM offers WHERE driver_id = $1', [
    userId,
  ]);
  ...
  const userResult = await query('SELECT rating_avg, rating_count FROM users WHERE id = $1', [
    userId,
  ]);
  ...
  res.json({
    ...
    totalPendingBookings:
      parseInt(pendingAsDriverResult.rows[0].total) +
      parseInt(pendingAsPassengerResult.rows[0].total),
