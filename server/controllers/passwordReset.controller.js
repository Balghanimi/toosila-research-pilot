  ...
  try {
    ...
    if (!user) {
      return res.json({
        success: true,
        message:
          'إذا كان البريد الإلكتروني موجوداً، ستتلقى رسالة لإعادة تعيين كلمة المرور / If the email exists, you will receive a password reset link',
      });
    }
    ...
    if (newPassword.length < 5) {
      return res.status(400).json({
        ...
        error: {
          ...
          message:
            'كلمة المرور يجب أن تكون 5 أحرف على الأقل / Password must be at least 5 characters',
        },
      });
    }
