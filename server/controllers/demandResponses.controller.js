...
const createDemandResponse = asyncHandler(async (req, res) => {
  ...
  const existingResponse = await DemandResponse.existsByDemandAndDriver(demandId, driverId);
  ...
  if (availableSeats < demand.seats) {
    throw new AppError(`عدد المقاعد المتاحة غير كافٍ. الطلب يحتاج ${demand.seats} مقعد/مقاعد`, 400);
  }
...
const updateResponseStatus = asyncHandler(async (req, res) => {
  ...
  if (response.status !== 'pending') {
    throw new AppError(
      `هذا الرد ${response.status === 'accepted' ? 'مقبول' : response.status === 'rejected' ? 'مرفوض' : 'ملغي'} بالفعل`,
      400
    );
  }
  ...
  if (status === 'accepted') {
    const rejectedCount = await DemandResponse.rejectOtherResponses(response.demandId, id);
