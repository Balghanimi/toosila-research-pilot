...
async function runMigration() {
  ...
  try {
    ...
    const migrationPath = path.join(
      __dirname,
      '../database/migrations/013_add_demands_response_count.sql'
    );
    ...
    if (columnCheck.rows.length > 0) {
      ...
      console.log(
        `   Type: ${columnCheck.rows[0].data_type}, Default: ${columnCheck.rows[0].column_default}`
      );
    }
    ...
    sampleData.rows.forEach((row) => {
      console.log(
        `   - Demand ${row.id}: ${row.from_city} → ${row.to_city} (${row.response_count} responses)`
      );
    });
