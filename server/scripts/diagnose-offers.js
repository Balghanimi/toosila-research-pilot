...
async function diagnose() {
  try {
    ...
    console.log(
      `✅ Offers with >0 available: ${result.rows.filter((o) => o.calculated_available > 0).length}`
    );
    console.log(
      `⚠️  Offers with 0 available: ${result.rows.filter((o) => o.calculated_available === 0).length}`
    );
