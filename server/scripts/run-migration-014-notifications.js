...
async function runMigration() {
  ...
  try {
    ...
    const migrationPath = path.join(
      __dirname,
      '../database/migrations/014_cleanup_orphaned_notifications.sql'
    );
    ...
    console.log(
      `🗑️  Deleted: ${beforeCount.rows[0].count - afterCount.rows[0].count} orphaned notifications`
    );
    console.log('');
    ...
    triggerCheck.rows.forEach((row) => {
      console.log(
        `   - ${row.trigger_name} on ${row.event_object_table} (${row.event_manipulation})`
      );
    });
    console.log('');
    ...
    if (sampleNotifications.rows.length === 0) {
      ...
      sampleNotifications.rows.forEach((row) => {
        ...
        console.log(
          `   - ${row.type} | demand:${demandId} | booking:${bookingId} | read:${row.is_read}`
        );
      });
    }
    console.log('');
