...
class Rating {
  ...
  static async getByUserId(userId, page = 1, limit = 10) {
    ...
    const countResult = await query('SELECT COUNT(*) FROM ratings WHERE to_user_id = $1', [userId]);
  ...
  static async hasRated(rideId, fromUserId) {
    const result = await query('SELECT id FROM ratings WHERE ride_id = $1 AND from_user_id = $2', [
      rideId,
      fromUserId,
    ]);
