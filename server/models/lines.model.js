...
const getAllLines = async (filters = {}) => {
  ...
  // Count total
  const countResult = await query(`SELECT COUNT(*) FROM (${queryStr}) as filtered`, params);
...
const updateAvailableSeats = async (lineId) => {
  ...
  if (line && line.available_seats <= 0) {
    await query(`UPDATE lines SET status = 'full' WHERE id = $1`, [lineId]);
  } else if (line && line.available_seats > 0) {
    await query(`UPDATE lines SET status = 'active' WHERE id = $1 AND status = 'full'`, [lineId]);
  }
...
const removeStop = async (stopId) => {
  const result = await query(`DELETE FROM line_stops WHERE id = $1 RETURNING *`, [stopId]);
