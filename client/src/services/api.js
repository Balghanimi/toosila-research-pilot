...
export const notificationsAPI = {
  ...
  // تسجيل رمز FCM للإشعارات الدفعية
  registerFcmToken: async (token, deviceInfo = null) => {
...
export const emergencyAPI = {
  ...
  getContacts: async () => {
    return apiRequest('/emergency/contacts', { method: 'GET' });
  },
  ...
  deleteContact: async (contactId) => {
    return apiRequest(`/emergency/contacts/${contactId}`, { method: 'DELETE' });
  },
};
...
export const sosAPI = {
  ...
  getMyAlerts: async (page = 1, limit = 10) => {
    return apiRequest(`/sos/my-alerts?page=${page}&limit=${limit}`, { method: 'GET' });
  },

  // Cancel an active SOS alert
  cancelAlert: async (alertId) => {
    return apiRequest(`/sos/${alertId}/cancel`, { method: 'PATCH' });
  },

  // Admin: Get all alerts
  adminGetAll: async (page = 1, limit = 20, filters = {}) => {
    const queryParams = new URLSearchParams({ page, limit, ...filters }).toString();
    return apiRequest(`/sos/admin/all?${queryParams}`, { method: 'GET' });
  },

  // Admin: Get active alerts
  adminGetActive: async (page = 1, limit = 50) => {
    return apiRequest(`/sos/admin/active?page=${page}&limit=${limit}`, { method: 'GET' });
  },

  // Admin: Get SOS stats
  adminGetStats: async () => {
    return apiRequest('/sos/admin/stats', { method: 'GET' });
  },

  // Admin: Acknowledge alert
  adminAcknowledge: async (alertId) => {
    return apiRequest(`/sos/admin/${alertId}/acknowledge`, { method: 'PATCH' });
  },

  // Admin: Resolve alert
  adminResolve: async (alertId, resolutionNotes) => {
    return apiRequest(`/sos/admin/${alertId}/resolve`, {
      method: 'PATCH',
      body: JSON.stringify({ resolutionNotes }),
    });
  },

  // Admin: Mark as false alarm
  adminFalseAlarm: async (alertId, notes) => {
    return apiRequest(`/sos/admin/${alertId}/false-alarm`, {
      method: 'PATCH',
...
// Survey API (Research Pilot)
export const surveyAPI = {
  // Give research consent
  giveConsent: async () => {
    return apiRequest('/survey/consent', { method: 'POST' });
  },

  // Check survey completion status
  getStatus: async () => {
    return apiRequest('/survey/status', { method: 'GET' });
  },

  // Get user's own surveys
  getMySurveys: async () => {
    return apiRequest('/survey/my-surveys', { method: 'GET' });
  },

  // Submit a survey (pre, post, or weekly)
  ...
  getStats: async () => {
    return apiRequest('/survey/stats', { method: 'GET' });
  },

  // Admin: export survey data
  exportData: async (type = null) => {
    const params = type ? `?type=${type}` : '';
    return apiRequest(`/survey/export${params}`, { method: 'GET' });
  },
};

// Analytics API (Research Pilot)
export const analyticsAPI = {
  // Track a single event
  track: async (eventType, eventCategory, properties = {}) => {
    return apiRequest('/analytics/track', {
      ...
      body: JSON.stringify({
        ...
        eventCategory,
        properties,
        sessionId: analyticsAPI._getSessionId(),
        platform: 'web',
      }),
    });
  },

  // Track a batch of buffered events
  ...
  // Admin: export events
  exportData: async (startDate = null, endDate = null) => {
    const params = new URLSearchParams();
    if (startDate) params.append('startDate', startDate);
    if (endDate) params.append('endDate', endDate);
    const qs = params.toString();
    return apiRequest(`/analytics/export${qs ? '?' + qs : ''}`, { method: 'GET' });
  },

  // Session ID helper (persists per browser session)
  _getSessionId: () => {
    let sessionId = sessionStorage.getItem('toosila_session_id');
    if (!sessionId) {
      sessionId = crypto.randomUUID();
      sessionStorage.setItem('toosila_session_id', sessionId);
    }
    return sessionId;
  },
};
