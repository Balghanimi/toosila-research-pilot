...
const OfferCard = ({ offer, onBookNow, onMessageDriver, formatDate, formatTime, currentUser }) => {
  ...
  return (
    <div className={styles.offerCard}>
      {/* Ladies-Only Badge */}
      {(offer.isLadiesOnly || offer.is_ladies_only) && (
        <div className={styles.ladiesOnlyBadge}>
          <span>👩</span>
      ...
      <div className={styles.routeSection}>
        ...
        {(offer.fromAddress || offer.toAddress) && (
          ...
          >
            {offer.fromAddress && (
              <div>
                <span style={{ color: '#16a34a' }}>&#9679;</span> {offer.fromAddress}
              </div>
            )}
            {offer.toAddress && (
              <div>
                <span style={{ color: '#ef4444' }}>&#9679;</span> {offer.toAddress}
              </div>
            )}
          </div>
        )}
