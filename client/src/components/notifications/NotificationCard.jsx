...
function NotificationCard({ notification, onDelete, onMarkAsRead }) {
  ...
  const handleAccept = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_URL || 'http://localhost:5001/api'}/bookings/${bookingId}/accept`,
        {
          method: 'POST',
          headers: {
  ...
  const handleReject = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.REACT_APP_API_URL || 'http://localhost:5001/api'}/bookings/${bookingId}/reject`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleCardClick = async () => {
    ...
    // إذا كان حجز جديد، انتقل لصفحة الحجوزات الواردة
